        ...
        case -1:
            ...
            else if (res == crushed)
            {
                switch (door->type)
                {
                    ...
                    default:
                        door->direction = 1;
                        // JoshK: Fast doors reopening with wrong sound bug fix
...
//
int32_t EV_DoLockedDoor(line_t *line, vldoor_e type, mobj_t *thing)
{
    player_t    *p;
...
int32_t EV_DoDoor(line_t *line, vldoor_e type)
{
    int32_t     secnum, rtn;
    sector_t    *sec;
    vldoor_t    *door;
...
//
void P_SpawnDoorRaiseIn5Mins(sector_t *sec, int32_t secnum)
{
    vldoor_t    *door;
