...
int32_t    key_map_north     = KEY_UPARROW;
int32_t    key_map_south     = KEY_DOWNARROW;
int32_t    key_map_east      = KEY_RIGHTARROW;
int32_t    key_map_west      = KEY_LEFTARROW;
int32_t    key_map_zoomin    = '=';
int32_t    key_map_zoomout   = '-';
int32_t    key_map_toggle    = KEY_TAB;
int32_t    key_map_maxzoom   = '0';
int32_t    key_map_follow    = 'f';
int32_t    key_map_grid      = 'g';
int32_t    key_map_mark      = 'm';
int32_t    key_map_clearmark = 'c';
...
mline_t    thintriangle_guy[] = {
    {{(fixed_t)(-0.5 * R), (fixed_t)(-0.7 * R)}, {(fixed_t)(R), (fixed_t)(0)}},
    {{(fixed_t)(R), (fixed_t)(0)}, {(fixed_t)(-0.5 * R), (fixed_t)(0.7 * R)}},
    {{(fixed_t)(-0.5 * R), (fixed_t)(0.7 * R)},
        {(fixed_t)(-0.5 * R), (fixed_t)(-0.7 * R)}}
};
#undef R
...
void AM_updateLightLev(void)
{
    ...
    //static int32_t    litelevels[] = {0, 3, 5, 6, 6, 7, 7, 7};
    static int32_t    litelevels[] = {0, 4, 7, 10, 12, 14, 15, 15};
    static int32_t    litelevelscnt = 0;
...
//
void AM_drawFline(fline_t *fl, int32_t color)
{
    register int32_t    x;
    register int32_t    y;
    register int32_t    dx;
...
//
void AM_drawMline(mline_t *ml, int32_t color)
{
    static fline_t    fl;
...
void AM_drawLineCharacter(mline_t *lineguy, int32_t lineguylines,
                          fixed_t scale, angle_t angle, int32_t color,
                          fixed_t x, fixed_t y)
{
    int32_t    i;
    mline_t    l;
    ...
    if (!netgame)
    {
        if (cheating)
        {
            AM_drawLineCharacter(cheat_player_arrow,
                                 arrlen(cheat_player_arrow), 0, plr->mo->angle,
                                 WHITE, plr->mo->x, plr->mo->y);
        }
        else
        {
            AM_drawLineCharacter(player_arrow,
                                 arrlen(player_arrow), 0, plr->mo->angle,
                                 WHITE, plr->mo->x, plr->mo->y);
        }
        return;
    }
...
void AM_drawThings(int32_t colors, int32_t colorrange)
{
    int32_t    i;
    mobj_t     *t;
